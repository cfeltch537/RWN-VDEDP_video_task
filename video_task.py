...
sharing_question = visual.TextStim(
    win, text="How likely would you be to share this clip with a friend?", pos=[0, 3], height=1.1)

ratingStim = []
xpos = [-8, -4, 0, 4, 8]
for rating in (1, 2, 3, 4, 5):
    ratingStim.append(visual.TextStim(win, text='%i' %
                                      rating, pos=(xpos[rating - 1], -3)))
...
def play_video(filename):
    ...
    logging.log(level=logging.DATA, msg='STARTING video {} dur {}\tsys:{}'.format(
        filename, video_duration, time.time()))
