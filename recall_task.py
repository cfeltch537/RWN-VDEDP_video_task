...
logger = logging.LogFile('logs/recall_test.log',
                         level=logging.DATA, filemode='w')
...
ready_prompt = visual.TextStim(
    win, text='Please start this task once you have driven onto the highway', pos=(0, 2), height=1.3)

instruction_block = visual.TextStim(
    win, text='', pos=(0, 3), wrapWidth=26, height=1.1)

next_label = visual.TextStim(
    win, text='[Press space to continue]', pos=(0, -4), height=0.8)
...
tell_the_driver = visual.TextStim(
    win, text='Tell the driver about this now', pos=(0, -2.5), height=1)

recog_question = visual.TextStim(
    win, text='Have you seen this item before?', pos=(0, -2.5), height=1)
recog_text = visual.TextStim(
    win, text='', wrapWidth=24, pos=(0, 4), height=1.1)
...
def do_fixation(fix_length, clock=localClock):
    ...
    logging.log(level=logging.DATA, msg='FIXATION dur {}\tsys:{}'.format(
        fix_length, time.time()))
...
def show_instructions(display_time=None):
    ...
    for idx, instruct_text in enumerate(instructions):
        ...
        logging.log(level=logging.DATA, msg='Showing instruction screen {}\tsys:{}'.format(
            idx + 1, time.time()))
...
def ready_screen():
    ...
    logging.log(level=logging.DATA,
                msg='*********** TASK STARTED *************\tsys:{}'.format(time.time()))
...
def open_recall(subj_id='test'):
    ...
    open_recall_audio_filename = os.path.join(
        'audio', 'open_recall_{}.wav'.format(subj_id))
    ...
    logging.log(level=logging.DATA,
                msg='START open recall\tsys:{}'.format(time.time()))
    ...
    logging.log(level=logging.DATA,
                msg='END open recall\tsys:{}'.format(time.time()))
    logging.flush()
...
def closed_recall(subj_id='test'):
    ...
    logging.log(level=logging.DATA,
                msg='START closed recall\tsys:{}'.format(time.time()))

    for row in stimuli_list:

        logging.log(level=logging.DATA,
                    msg='stim - {}\tsys:{}'.format(row['file2'], time.time()))

        actor_img.setImage('img/actor{}.png'.format(row['actor']))
        prompt_text.setText(row['prompt'])

        closed_recall_audio_filename = "audio/closed_recall_{}_{}.wav".format(subj_id, row[
                                                                              'file2'])

        tr = CountDownTimer(closed_recall_duration)

        mic.record(closed_recall_duration + 5,
                   filename=closed_recall_audio_filename)
        ...
        mic.reset()
    logging.log(level=logging.DATA,
                msg='END closed recall\tsys:{}'.format(time.time()))
    logging.flush()
...
def closed_recall_v2(subj_id='test'):
    ...
    for recog_item in recog_list:
        ...
        logging.log(level=logging.DATA, msg="Recognition item: {} - {}\tresp: {}".format(
            recog_item['code'], recog_item['text'], resp))

    old_items = [i for i in recog_list if not i['code'].endswith('n')]
    for item in old_items:
        recog_text.setText(item['text'])

        closed_recall_audio_filename = "audio/closed_recall_{}_{}.wav".format(
            subj_id, item['code'])
...
if __name__ == "__main__":
    ...
    logging.log(level=logging.DATA,
                msg='*********** TASK ENDED *************\tsys:{}'.format(time.time()))
